...
- **About (81) — FAQ full-width (15/7)**: la FAQ era in una inner-section boxed con colonna col-50 (~640px, metà larghezza) + colonna gemella vuota (residuo del CTA spostato). Portata la colonna FAQ (bb9976d) a `_column_size:100/_inline_size:100` ed eliminata la colonna vuota (91103f2). Ora la FAQ riempie la larghezza contenuto (~1120px), allineata al blocco CTA sotto (~1106px).

- **About (81) — servizi ridotti a 8 (15/7)**: rimossi i servizi non pertinenti al pubblico plugin/software (Advertising, Social Media Management, Logo & Brand Identity, Graphic Design, Photography, NFT & Blockchain, Visual Merchandising). Tenuti Website Development, iOS & Android Apps, E-commerce Solutions, Digital Marketing, AI Solutions, Growth Hacking, CRO + "Video & Animations" rinominato **Video AI** (descrizione: "AI-generated videos and animations..."). Le immagini dei servizi rimossi restano in media library (non usate). Widget features #ee127a4.

- **About (81) — servizi ridotti a 6 (15/7, ripensamento)**: tolti anche Video AI e Digital Marketing. Restano 6 voci in griglia 3+3: Website Development, iOS & Android Apps, E-commerce Solutions, AI Solutions, Growth Hacking, CRO. Widget features #ee127a4.

- **Documentation (4637) — fix requisito WP (15/7)**: corretto "WordPress 7.0 or newer (7.1+ unlocks...)" → "WordPress 7.1 or newer (it powers the native AI Client, semantic search and document indexing)" in Requisiti (widget 8043953) e "below WordPress 7.0" → "7.1" in Troubleshooting (22e09e7). Il readme.txt ufficiale del plugin dice "Requires at least: 7.1". Resto della doc verificato OK (indice/ancore funzionanti, privacy con framing corretto, contenuti coerenti).

- **Footer — colonna Product (15/7)**: la colonna Product del footer è il menu WordPress **"What We Do" (id 16)** (widget Nav Menu). Aggiunte 5 voci custom sotto Bot Marley/Support, con target _blank: Billing (customers.freemius.com/login), License Recovery (dashboard.freemius.com/license-recovery/33359/bot-marley/), Refund Policy + Price Policy + EULA (freemius.com/product/33359/bot-marley/legal/eula/ con anchor #refund_policy_summary / #subscriptions_summary). La colonna Company = menu "Other Pages" (id 14).

## Supporto — Help Scout (21/7)

- Account Help Scout **BigFive** (piano Plus, trial). Docs site **"Bot Marley"** (id `6a5fb6c9e648f10eb28f2c8c`, settings site id `6a5fb53f46f1b4bed9565cf1`), collezione **"Bot Marley"** pubblica.
- Caricati **14 articoli** = le 14 sezioni della guida utente (da `_handoff/testi/Bot-Marley-User-Guide.md`), in inglese, formattati. **Tutti pubblicati** e messi nella categoria **"Bot Marley User Guide"** (id `6a5fc4163b1dd83f6deeff71`), ordine "Custom" = Welcome->Troubleshooting.
- Nota tecnica editor Help Scout Docs: e Slate.js. NIENTE import HTML via file_upload (accetta solo file allegati dall utente) ne paste sintetico (Slate ignora eventi non-trusted). Metodo funzionante: **digitare markdown reale** (`## ` heading, `**grassetto**`, `- ` / `1. ` liste con trattino/numero solo sul PRIMO item, `> ` citazione, singolo \n tra blocchi). Requisito WP corretto a **7.1** (readme.txt del plugin).
- Integrazione Freemius↔Help Scout (dashboard.freemius.com .../integrations/helpscout/): chiavi API + Custom App **collegate da Lorenzo**. Le due parti: campo "Docs API Key" + Custom App "Dynamic Content" con Callback URL/Secret. (Io non posso inserire chiavi/secret nei campi.)

- **Decisione doc (21/7)**: la **documentazione pubblica ufficiale** e la pagina del sito **apps.bigfive.cloud/documentation/** (brandizzata, gia nel menu). Help Scout Docs resta **PRIVATO** (site "Public site" toggle OFF su bigfive.helpscoutdocs.com): NON pubblicare. Serve solo dietro le quinte come sorgente per **AI Answers** e riferimento nei ticket (+ eventuale Beacon). Da fare lato plugin (readme.txt/zip, non da qui): puntare il link Documentation del plugin a https://apps.bigfive.cloud/documentation/.

- **Routing supporto Freemius -> Help Scout (21/7, FUNZIONANTE e testato)**: su Freemius (dashboard -> Bot Marley -> **Emails -> Addresses**) tolta la spunta "Use store's email addresses" e impostato il campo **General Support = `help@bigfive.helpscoutapp.com`** (label "Help Scout" = la inbox Help Scout "Bot Marley"). Cosi le richieste dal form in-plugin e dal Customer Portal arrivano come conversazioni in Help Scout. Gli altri campi (General System Address/from, Don't Reply) lasciati vuoti = default. Testato: il ticket arriva. Le risposte ai clienti partono da Help Scout, non da Freemius.

- **Compatibilita ACF / page builder (21/7)**: aggiunte 2 FAQ nella sezione FAQ Bot Marley (sito, widget f7aa94b): "Is it compatible with Advanced Custom Fields (ACF)?" (SI, compatibile) e "Does it work with page builders like Elementor or Divi?" (NO: contenuto costruito con Elementor/Divi/builder frontend non viene indicizzato). Stessa nota **Compatibility** aggiunta nella sezione requisiti della doc sito (/documentation/, widget 8043953) e nell articolo Help Scout "What you need before you start" (id 6a5fbca30e7c59fe8910011c, ripubblicato). Framing: legge contenuto WP standard + ACF; non legge contenuto dei page builder frontend.

- **Compatibilita page builder — posizionamento rivisto (22/7, richiede plugin >= 0.9.10)**: sostituita la nota precedente in tutti e 3 i punti (FAQ sito widget f7aa94b, doc sito /documentation/ widget 8043953, articolo Help Scout "What you need" 6a5fbca30e7c59fe8910011c). Nuova matrice: Gutenberg/Classic/ACF (campi + ACF Blocks) = fully supported; **Elementor = fully supported** (pagine indicizzate per intero, esclusi Theme Builder templates e popup); Divi/WPBakery/Avada = partially supported (testo + contenuti chiave: titoli sezione, label bottoni, FAQ/accordion; alcuni elementi builder saltati); Bricks/Oxygen e simili = not fully compatible yet (solo titoli pagina). NB: aggiorna il precedente handoff del 21/7 che diceva Elementor/Divi non compatibili.

- **Box avviso AI provider sotto i prezzi (22/7)**: aggiunto un widget HTML (classe `.bf-ai-warn`) in fondo alla sezione pricing (369e990) di /bot-marley/, subito sotto le tariffe e prima di "Support & Customer Portal". Box giallo/oro (bg #fbf1c9, bordo-sx #ebbe1e) con emoji warning: avvisa che il plugin per funzionare si collega al proprio account OpenAI/Gemini/Claude via integrazione AI nativa di WordPress (Settings -> Connectors), la chiave non viene salvata, uso AI fatturato dal provider. CSS inline nel widget stesso.

## Documentazione riaggiornata alla guida 0.12.0 (22/7)

- **Fonte nuova:** `/Users/biglore/claude-code-workspace/Bot Marley/Bot-Marley-User-Guide.md` (Bot Marley **0.12.0**). Rimpiazza la snapshot obsoleta in `_handoff/testi/`.
- **Sito /documentation/ (page 4637): RIFATTO COMPLETO.** Tutte le 14 sezioni riscritte (contenuto HTML nei text-editor via Elementor API) + **NUOVA sezione "AI Site Search (Pro)"** (slug `#search`, heading h2 + text-editor, con 2 tabelle shortcode + code block) inserita tra WooCommerce e Settings. Indice (TOC) ora 15 voci. Intro con "Applies to 0.12.0". Novita coperte: creazione API key per Gemini/OpenAI/Anthropic con link, WhatsApp number, Excluded URLs, auto-open widget, Custom CSS, regole di sicurezza obbligatorie, transcript consent, proxy/CDN row, content-gap search stats, ecc.
- ⚠️ **Requisito WP tornato a "7.0 or newer (7.1+ unlocks...)"** come dice la guida 0.12.0 (annulla il fix a 7.1 del 15/7, che seguiva un readme vecchio).
- **Matrice compatibilita ≥0.9.10 MANTENUTA** in coda alla sezione Requisiti (non e nella guida ma e info voluta).
- **Help Scout: NON aggiornato (scelta di Lorenzo, "lascia per ora").** Resta la versione precedente; solo l articolo Welcome (6a5fba4c...) e stato riaggiornato. Da rifare quando si potra importare il file (file_upload accetta solo file allegati in chat; Docs API key non maneggiabile; editor Slate tronca i testi lunghi).

- **Sottomenu indice sotto Connect your AI (22/7)**: le 4 sotto-intestazioni della sezione connect-ai (widget 65606e2) portate da h4 a **h3** con ancore leggibili + titoli brevi: #connect-gemini "Connect Google — Gemini", #connect-openai "Connect OpenAI — ChatGPT", #connect-anthropic "Connect Anthropic — Claude", #connect-wordpress "Add the key to WordPress". Widget TOC (b7f909d) impostato `headings_by_tags:[h2,h3]` → Elementor annida come 4.1/4.2/4.3/4.4 SOLO sotto Connect your AI (le altre sezioni usano h4, non entrano nel TOC). CSS Customizer `/* doc-h3 */` per stile h3 + scroll-margin-top:150px (offset ancore).
