...
5. **Header a due livelli** (14/7): 
   - Colori/loghi (ultima versione): **topbar gialla** `#ebbe1e` — logo **BigFive Apps black** piccolo a sinistra (nello script), link **Bot Marley · About · Support** grigio scuro a destra (`.bf-topbar__nav`, layout `space-between`). **Menu bar bianca** con logo **Bot Marley** (`logo-bot-marley.svg` su Kirki `logo`+`seconday_logo`), voci scure `#252427` (hover viola), **BUY PRO** invertito viola/bianco (`#header-sticky .tp-btn`).
